...
export default function AdminClientsClient({
  ...
  initialClients: ClientWithStatus[];
  initialLoadError?: string;
}) {
  const [clients, setClients] = useState<ClientWithStatus[]>(initialClients);
  ...
  async function fetchClients() {
    ...
    if (res.ok) {
      ...
      setLoadError(data.error ?? "Could not refresh clients. Please try again.");
    }
  }
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 p-6 space-y-4 animate-fade-in">
        {loadError && (
          <div className="flex items-center justify-between gap-3 rounded-lg border border-red-400/30 bg-red-400/10 px-3 py-2 text-sm text-red-300">
            <span>{loadError}</span>
            <button onClick={fetchClients} className="text-xs font-semibold underline cursor-pointer">Retry</button>
          </div>
        )}
        {/* Search */}
