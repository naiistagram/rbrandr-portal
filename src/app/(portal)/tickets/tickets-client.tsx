...
interface Props {
  ...
  userId: string;
  /** Read-only admin preview: hides ticket creation and reply UI. */
  preview?: boolean;
}

export function TicketsClient({ initialTickets, initialProjectId, userId, preview = false }: Props) {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {!preview && <Topbar title="Tickets" subtitle="Raise and track support issues" userId={userId} />}
      ...
      <div className="flex-1 p-6 space-y-5 animate-fade-in">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            {!preview && <p className="text-xs text-[var(--foreground-subtle)] mt-1">Submit a ticket if you need any help</p>}
          </div>
        ) : (
                ...
                <div key={ticket.id} className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Client reply */}
                      {!preview && ticket.status !== "resolved" && ticket.status !== "closed" && (
