...
interface Props {
  ...
  userId: string;
  /** Read-only admin preview: hides content requests and approve/reject actions. */
  preview?: boolean;
}
...
export function ContentClient({ initialItems, initialProjectId, userId, preview = false }: Props) {
    ...
    return (
      <button
        ...
        className={cn(
          ...
          item.status === "in_review" && item.review_note && "border-[var(--accent)] shadow-lg shadow-[var(--accent)]/10"
        )}
      >
        <div className={cn(
        ...
        <div className="p-3 space-y-2">
          ...
          {item.status === "in_review" && item.review_note && (
            <div className="flex items-center gap-1.5 rounded-md bg-[var(--accent-subtle)] px-2 py-1.5 text-[10px] font-semibold text-[var(--accent)]">
              <MessageSquare className="w-3 h-3 flex-shrink-0" /> Note from your account manager
            </div>
          )}
        </div>
      </button>
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {!preview && <Topbar title="All Content" subtitle="View all content across your project" userId={userId} />}
          ...
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col md:flex-row overflow-hidden animate-fade-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col flex-1 min-h-0">
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                ...
                {selected.status === "in_review" && selected.review_note && (
                  <div className="rounded-xl border border-[var(--accent)]/45 bg-[var(--accent-subtle)] p-4 shadow-lg shadow-[var(--accent)]/10">
                    <p className="text-xs font-bold text-[var(--accent)] uppercase tracking-wider mb-2 flex items-center gap-1.5"><MessageSquare className="w-3.5 h-3.5" /> Your account manager needs a check</p>
                    <p className="text-sm font-medium text-[var(--foreground)] leading-relaxed whitespace-pre-wrap">{selected.review_note}</p>
                  </div>
                )}
                ...
                {selected.status === "in_review" && (
                  preview ? (
                    <div className="p-3 rounded-lg bg-amber-400/10 border border-amber-400/20 flex items-center gap-2 text-xs text-amber-400">
                    ...
                    <div className="space-y-3 pt-2">
                      ...
                      <textarea
                        ...
                        placeholder={selected.review_note ? "Reply to the review note or add feedback…" : "Add feedback or notes (optional)…"}
                        rows={3}
                ...
                {selected.status === "approved" && (
                  <div className="p-3 rounded-lg bg-emerald-400/10 border border-emerald-400/20 flex items-center gap-2 text-xs text-emerald-400">
                    ...
                    {preview ? "The client approved this content." : "You approved this content."}
                  </div>
                )}
                ...
                {selected.status === "rejected" && (
                  <div className="p-3 rounded-lg bg-red-400/10 border border-red-400/20 flex items-center gap-2 text-xs text-red-400">
                    ...
                    {preview ? "The client rejected this content." : "You rejected this content."}
                  </div>
                )}
              </div>
            </div>
